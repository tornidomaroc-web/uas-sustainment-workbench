...
class WorkOpen(_Payload):
    work_id: str | None = Field(default=None, min_length=1, max_length=64)
    state: WorkStateIn


class WorkStateChange(_Payload):
    work_id: str = Field(min_length=1, max_length=64)
    state: WorkStateIn


class WorkClose(_Payload):
    work_id: str = Field(min_length=1, max_length=64)


class ComponentRegister(_Payload):
...
class ComponentMove(_Payload):
    aircraft_key: str = Field(min_length=1)


class InspectionDoneIn(_Payload):
...
def _shape(entry: Entry, now: datetime) -> dict[str, Any]:
    ...
    try:
        model = PAYLOADS[entry.kind].model_validate(entry.payload)
    except ValidationError as exc:
        problems = "; ".join(
            f"{'.'.join(str(p) for p in err['loc']) or 'payload'}: {err['msg']}"
            for err in exc.errors()
        )
        raise LedgerError(422, f"invalid payload for {entry.kind}: {problems}") from exc
    return model.model_dump(mode="json")


def _subjects(store: Store, entry: Entry, projection: Any, payload: dict[str, Any]) -> None:
...
def _dependents(target: Entry, live: tuple[Entry, ...]) -> list[int]:
    ...
    if target.kind == "work_order.open":
        wid = target.payload.get("work_id")
        return [
            e.id
            for e in same
            if e.kind in ("work_order.state", "work_order.close")
            and e.payload.get("work_id") == wid
            and e.id
        ]
    if target.kind == "component.install":
        later = [
            ...
            and e.payload.get("aircraft_key") == target.payload.get("aircraft_key")
        ]
...
def _transitions(
    ...
    if entry.kind == "work_order.open":
        ...
        if not wid:
            payload["work_id"] = f"WO-{store.next_entry_id()}"
    elif entry.kind in ("work_order.state", "work_order.close"):
        wid = str(payload["work_id"])
        ...
        if payload["kind"] not in policy.component_kinds:
            raise LedgerError(
                422,
                f"component kind {payload['kind']!r} is not in fleet.toml; the kinds are "
        ...
        if payload["name"] not in policy.inspections:
            raise LedgerError(
                422,
                f"inspection {payload['name']!r} is not in fleet.toml; the inspections are "
        ...
        if payload.get("at_hours_s") is None:
            payload["at_hours_s"] = round(tis, 1)
            payload["derived"] = True
        elif payload["at_hours_s"] > tis + 1.0:
            raise LedgerError(
                422,
                f"at_hours_s {payload['at_hours_s'] / 3600:.1f} h is more than the time in "
...
def validate(
    ...
    *,
    ...
    payload = _shape(entry, now)
    projection = store.projection()
    _subjects(store, entry, projection, payload)
    ...
    payload = _transitions(store, entry, payload, projection, policy, tolerance_s)
    statement = entry.statement.strip() or (entry.reason or "").strip()
    return dataclasses.replace(entry, payload=payload, statement=statement)
